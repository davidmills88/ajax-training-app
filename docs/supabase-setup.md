...
If `DATABASE_URL` is set but Postgres is unreachable, the API logs a warning and stays on the in-memory mock. Live Supabase / pooler URLs use `ssl: { rejectUnauthorized: false }` automatically — do not set `NODE_TLS_REJECT_UNAUTHORIZED=0`. Public deploy: [go-live.md](./go-live.md).
